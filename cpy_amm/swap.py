...
class MarketQuote:
    def __init__(self, trading_pair: str, price: float):
        ...
        Args:
            trading_pair (str): The trading pair ticker eg. ETH/USD
...
def add_liquidity(
    ...
    """Adds a given amount of liquidity in the AMM
...
def assert_cp_invariant(x: float, y: float, k: float, precision: float | None = None):
    ...
    Args:
        k (float) : Constant product
        x (float) : Reserve of tokens A
        y (float) : Reserve of tokens B
        precision (float) : Precision at which the invariant is evaluated
...
def constant_product_swap(
    ...
    Args:
        dx (float) : Amount of tokens A in
        x (Pool) : Pool representing the reserve of tokens A
        y (float) : Pool representing the reserve of tokens B

    Returns:
        Tuple[float, float] : (Amount of tokens B out, Swap execution price)
...
def swap_price(x: float, y: float, dx: float) -> float:
    """Computes the swap execution price for an order size
    given two pools with reserves x and y
...
def constant_product_curve(
    ...
    Args:
        pool_1 (Pool) : liquidity pool 1 eg. X
        pool_1 (Pool) : liquidity pool 2 eg. Y
        k (float) : constant product invariant
        x_min (float) : minimum value of X
        x_max (float) : maximum value of X
        num (float) : number of points to be computed

    Returns:
        Tuple[list[float],list[float]] : (X, Y = K/X)
    ...
    num = num or 1000
    x = np.linspace(x_min, x_max, num=int(num))
    y = k / x
    return x, y
...
def price_impact_range(
    ...
    """Returns the range of prices that can be achieved
    by swapping dx amount of tokens A for dy amount of tokens B
...
def order_book(
    ...
    """Computes the cumulative quantity at any mid price according to the formula
    from the paper "Order Book Depth and Liquidity Provision in Automated Market Makers"
